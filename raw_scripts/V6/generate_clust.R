...
generate_clust <- function(output_file_name,
  ...
  install_HDBSCAN <-
    function(...,
      ...
      reticulate::py_install(packages = c("hdbscan"), envname = envname,...)

      .onLoad <- function(...) {
        use_virtualenv("r-HDBSCAN", required = TRUE)

      reticulate::source_python(system.file("python", "clus.py", package = "ChIPbinner"))
      }
    }
  install_HDBSCAN()
  use_virtualenv(virtualenv = "r-HDBSCAN", required = TRUE)
  reticulate::source_python(file=system.file("python", "clus.py", package = "ChIPbinner"),envir = r-HDBSCAN)
