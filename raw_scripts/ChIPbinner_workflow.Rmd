...
genic_intergenic_scatterplot(
  path_for_norm_bw = "/path_for_normalized_bigwig",
  out_dir = "/output_directory",
  gene = gene,
  intergenic = igr,
  cell_line = "Cal27",
...
Prior to generating clusters, we need to pre-process the normalized bigWig files. This is accomplished using the <code>pre_clus()</code> function. 
```{r}
```

## Running HDBSCAN to generate clusters

The <code>clust()</code> function can be used to generate clusters of similarly-behaving genomic bins. It is based on an unsupervised learning algorithm to find clusters, or dense regions, of a dataset. More details on the Hierarchical Density-Based Spatial Clustering of Applications with Noise (HDBSCAN) algorithm can be found <a href="https://hdbscan.readthedocs.io/en/latest/index.html">here</a>.

However, it is important to consider this step will require substantial processing memory as well as time. Thus, it is recommended that you use the python-implementation of HDBSCAN in a High-performance Computing server instead. The workflow for either method is given below.

### Using a python-implementation of HDBSCAN
```{python,eval=FALSE}
import 
python clus.py --samp ${samp} 
              --csvFile ${csvFile} --minpts 5000 --minsamps 5000 --xlab ${xlab} --ylab ${ylab} --logfile ${log}
echo "DBSCAN end"
```

### Using HDBSCAN within R
```{r,eval=FALSE}
