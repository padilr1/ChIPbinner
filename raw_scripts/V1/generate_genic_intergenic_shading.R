...
# start of code
generate_genic_intergenic_scatterplots <- function(wd,path,gene,intergenic,cell_line,treatment,control,histone_mark,window_size,title_of_plot,max_x,max_y){
setwd(wd)
getwd()
path <- paste0(path)
  ...
  lapply(function(x) {
    ...
    pm <- ggplot() +
      ...
      labs(x = sprintf("%s \u25ba",control),
           y = sprintf("%s \u25ba",treatment),title=title_of_plot) +
