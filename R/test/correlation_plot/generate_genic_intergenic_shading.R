...
# max_y=30
# start of code
generate_genic_intergenic_scatterplots <- function(wd,path,gene,intergenic,cell_line,treatment,control,histone_mark,window_size,title_of_plot,max_x,max_y,min_x=NULL,min_y=NULL){
setwd(wd)
getwd()
path <- paste0(path)
...
#
#
d[[control]] <- subsetByOverlaps(d[[control]],d[[treatment]])
d[[treatment]]<- subsetByOverlaps(d[[treatment]],d[[control]])
#
lapply(d,length)
#
#
...
leg <- ggplot() +
  ...
  theme(panel.background = element_blank(),
        ...
        axis.title = element_text(family = 'Arial',
                                  ...
                                  size = 10))
leg
ggsave(filename = "figs/legend.png", plot=leg, height = 1, width = 1,device = "png",dpi=600,bg = "white")
# make legend
...
ps <- split(d, s$line) %>%
  lapply(function(x) {
    ...
    #                     scales::expand_range(mul = .05))
    if(is.null(min_x)){
      min_x = min(pdat2$x[pdat2$count > 10])} else {
        min_x = as.numeric(paste0(min_x))
      }
    if(is.null(min_y)){
      min_y = min(pdat2$y[pdat2$count > 10])} else {
    ...
    pm <- ggplot() +
      ...
      coord_cartesian(expand = F,
                      ...
                      ylim = lim$y) +
      # facet_grid(.~ttl) +
      labs(x = "H3K36me2 log2FC (NSD2i/Vehicle)",
           y = "H3K27me3 log2FC (NSD2i/Vehicle)",title=title_of_plot) +
      # geom_smooth(method='lm',inherit.aes = FALSE,aes(x=x,y=y),data=tdat) +
      # annotate('segment', x = -Inf, xend =Inf, y = Inf,
      #          yend = Inf, color = 'white') +
      geom_vline(xintercept = 0,linetype="dashed",
                 color = "black", size=0.3,alpha=0.1) +
    ...
    #            xlim = lim$x,
    #            ylim = lim$y) +
    #   labs(x = sprintf("%s \u25ba",control),
    #        y = sprintf("%s \u25ba",treatment)) +
    #   facet_grid(. ~ ttl) +
...
ps[[cell_line]]
ggsave((sprintf('figs/%s.%s.%s.%s.scatterplot.png',cell_line,control,treatment,mark)), ps[[cell_line]], height = 4, width = 4, device = "png",dpi=600)
# create directory for plot data
out_dir = paste0(wd,"/data/plot_data")
if (!file.exists(out_dir)) {
  dir.create(out_dir)
}
# if (!file.exists("data/plot_data")) {
#   dir.create("data/plot_data")
# }
library(gdata)
# rename object using library gdata
mv(from = "ps", to = paste0(sprintf('%s.%s.%s.%s.scatterplot',cell_line,control,treatment,mark)))
#assign((sprintf('%s.%s.%s.%s.clus.fig',cell_line,control,treatment,mark)), ps)
save(list=paste0(sprintf('%s.%s.%s.%s.scatterplot',cell_line,control,treatment,mark)),file=(sprintf('data/plot_data/%s.%s.%s.%s.scatterplot_data.rda',cell_line,control,treatment,mark)))
# save legend as data
save(leg,file = "data/plot_data/legend.RData")
}
