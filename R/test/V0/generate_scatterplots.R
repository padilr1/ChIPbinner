...
library(isoband)
# start of function
generate_scatterplots <- function(wd,path_for_norm_bw,treatment,control,cell_line,histone_mark,gene,intergenic,title_of_plot,window_size,number_of_clusters){
# parameters
setwd(wd)
getwd()
...
save(ps,file=(sprintf('figs/%s.%s.%s.%s.clus.fig.rda',cell_line,control,test,mark)))

ggsave((sprintf('figs/%s.%s.%s.%s.clus.pdf',cell_line,control,test,mark)), ps[[cell_line]], height = 2.4, width = 6.2, device = cairo_pdf)
  } else {
    clus <- case_when(
      overlapsAny(r, cons$A) ~ 'A',
      overlapsAny(r, cons$B) ~ 'B',
    ...
    #assign((sprintf('%s.%s.%s.%s.clus.fig',cell_line,control,test,mark)), ps)
    save(ps,file=(sprintf('plots/%s.%s.%s.%s.clus.fig.rda',cell_line,control,test,mark)))
