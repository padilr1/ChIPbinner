...
#' @param matrix_file The matrix file generated from 'pre_clus()', consisting of only the enrichment scores per bin for each of the two samples being compared.
...
density_based_scatterplot <- function(path_for_norm_bw,
  ...
  # read in files
  mat$clu <- read_csv(file = hdbscan_output_file, col_names = F)$X1
  ...
  # generating a density plot for a comparison with 2 clusters
  if (number_of_clusters == 2) {
    clus <- case_when(
    ...
    ggsave((sprintf("%s/%s.%s.%s.%s%sclus.pdf", out_dir,cell_line, baseline_samp_label, treated_samp_label, mark, window_size)), ps[[cell_line]], height = height_of_figure, width = width_of_figure, dpi = 600, units = "cm", device = cairo_pdf)
    ggsave((sprintf("%s/%s.%s.%s.%s%sclus.png", out_dir, cell_line, baseline_samp_label, treated_samp_label, mark, window_size)), ps[[cell_line]], height = height_of_figure, width = width_of_figure, device = "png", dpi = 600, units = "cm")
  } else {
    clus <- case_when(
      overlapsAny(r, cons$A) ~ "A",
      overlapsAny(r, cons$B) ~ "B",
    ...
    ggsave((sprintf("%s/%s.%s.%s.%s%sclus.pdf", out_dir,cell_line, baseline_samp_label, treated_samp_label, mark, window_size)), ps[[cell_line]], height = 2.4, width = 6.2, device = cairo_pdf)
  }
  ggsave((sprintf("%s/%s.%s.%s.%s%sclus.png",out_dir, cell_line, baseline_samp_label, treated_samp_label, mark, window_size)), ps[[cell_line]], height = height_of_figure, width = width_of_figure, device = "png", dpi = 600, units = "cm")
