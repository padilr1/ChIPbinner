...
Only toy datasets, which have been largely downsampled, are included with the package. For complete example datasets please see <a href="https://github.com/padilr1/ChIPbinner_database.git">ChIPbinner database</a>. Included in the ChIPbinner database are complete H3K36me2 ChIP-seq samples binned into 10kb windows, normalized bigWig files, the complete pooled BED file of genomic coordinates, the complete matrix file and finally, the complete clustering results from running HDBSCAN on the whole dataset. 

Also included in the ChIPbinner database are reference windows that can be used to transform aligned BAM files into binned BED files as well as curated databases for input into later sections of the workflow.
...
<b>It is important to consider this step will require substantial processing memory as well as time </b>. Thus, it is recommended that you use the python-implementation of HDBSCAN in a High-performance Computing server instead. Nevertheless, an example workflow for running HDBSCAN is found below. Using the complete matrix file for the density-based scatterplot generated below required substantial processing memory and multiple parallel cores, typically found in a high-performance computing cluster. The complete matrix file can be found at the <a href="https://github.com/padilr1/ChIPbinner_database.git">ChIPbinner database</a>.
```{r,eval=FALSE}
...
annotate_clust(number_of_clusters = 3,
               ...
               "clus.Cal27.WT.NSD1_KO.H3K36me2.10kb.5000.5000.txt", package = "ChIPbinner"),
               output_filename = "Cal27.WT.NSD1_KO.H3K36me2.10kb",
               out_dir="output_directory")
```
...
The <a href="https://github.com/padilr1/ChIPbinner_database.git">ChIPbinner database</a> contains curated databases ready for input into ChIPbinner. These databases are intended for input into the parameter <code>functional_db</code> within the <code>enrich_clust()</code> function.

<ul>
<li><code>ensemblDB</code> from <a href="https://useast.ensembl.org/info/genome/index.html">Ensembl</a></li>
<li> <code>ccreDB</code> from <a href="https://useast.ensembl.org/info/genome/index.html">ENCODE</a>,which are candidate cis-regulatory elements (cCREs)</li>
<li> <code>repeatsDB</code> from <a href="https://www.repeatmasker.org/">RepeatMasker</a></li>
</ul>
